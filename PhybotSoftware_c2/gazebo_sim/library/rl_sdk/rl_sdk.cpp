...
void RL::InitObservations()
{
    ...
    this->obs.last_lin_vel = torch::tensor({{0.0, 0.0, 0.0}});
    this->obs.ang_vel = torch::tensor({{0.0, 0.0, 0.0}});
    this->obs.gravity_vec = torch::tensor({{0.0, 0.0, -1.0}});
    this->obs.commands = torch::tensor({{0.0, 0.0, 0.0, 1.0}});
    ...
    this->obs.history_pos_buf = torch::zeros({1, this->params.history_len * this->params.num_of_dofs});
    this->obs.history_vel_buf = torch::zeros({1, this->params.history_len * this->params.num_of_dofs});
    this->obs.history_actions_buf = torch::zeros({1, this->params.history_len * this->params.num_of_dofs});
    this->obs.heights = torch::zeros({1, this->params.num_of_heights});
}
...
void RL::KeyboardInterface()
{
    if(kbhit())
    {
        ...
        switch(c)
        {
            ...
            case 'q': break;
            case 'w': this->control.x += 0.5; break;
            case 's': this->control.x -= 0.5; break;
...
void RL::ReadYaml(std::string robot_name)
{
    ...
    this->params.clip_actions_lower = config["clip_actions_lower"].as<double>();
    this->params.clip_actions_upper = config["clip_actions_upper"].as<double>();
    ...
    this->params.history_len = config["history_len"].as<int>();
    this->params.num_of_heights = config["num_of_heights"].as<int>();
    ...
    this->params.commands_scale = torch::tensor({this->params.lin_vel_scale, this->params.lin_vel_scale, this->params.ang_vel_scale, 1.0});
